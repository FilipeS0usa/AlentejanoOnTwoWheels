...
async function updateStats() {
  try {
    ...
    const stats = {
      ...
      totalDistance: trips.reduce((sum, trip) => sum + (parseInt(trip.distance) || 0), 0),
      totalPhotos: trips.length * 15, // Estimate photos per trip
      favoriteRoute: getFavoriteRoute(trips)
    };
    
    // Update stats with animation
    animateCounter("total-trips", stats.totalTrips);
    animateCounter("total-distance", stats.totalDistance);
    animateCounter("total-photos", stats.totalPhotos);
    
    const favoriteRouteElement = document.getElementById("favorite-route");
    if (favoriteRouteElement) {
      favoriteRouteElement.textContent = stats.favoriteRoute;
    }
...
// Get favorite route based on most recent or most popular
function getFavoriteRoute(trips) {
  if (trips.length === 0) return "Évora";
  
  // For now, return the most recent trip's title
  const sortedTrips = trips.sort((a, b) => new Date(b.date) - new Date(a.date));
  return sortedTrips[0].title.split(' ').slice(-1)[0]; // Get last word of title
}
